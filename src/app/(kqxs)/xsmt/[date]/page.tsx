// src/app/(kqxs)/xsmt/[date]/page.tsx
...
export async function generateMetadata({ params }: { params: Promise<{ date: string }> }): Promise<Metadata> {
    ...
    const draws = await prisma.draw.findMany({
        where: { lotteryType: { code: 'XSMT' }, drawDate, isComplete: true },
    ...
    return {
        title: `XSMT ${dateVN} — Kết quả xổ số Miền Trung — Thiên Số`,
        description: `Kết quả XSMT ngày ${dateVN}. ${dbList}. Bảng đầy đủ, lô tô, dò vé.`,
        openGraph: { title: `KQXSMT ${dateVN} — Thiên Số` },
        alternates: { canonical: `https://thienso.com/xsmt/${date}` },
    }
}
...
export default async function XSMTDatePage({ params }: { params: Promise<{ date: string }> }) {
    const { date } = await params
    ...
    const loganData = [
        { num: '84', days: 32, isHot: false },
        { num: '36', days: 28, isHot: false },
        { num: '12', days: 24, isHot: false },
        { num: '47', days: 20, isHot: false },
        { num: '58', days: 16, isHot: true },
        { num: '21', days: 12, isHot: true },
        { num: '63', days: 8, isHot: true },
        { num: '95', days: 4, isHot: true },
    ]
            ...
            <div className="flex gap-6">
                ...
                <main className="flex-1 space-y-5 min-w-0">
                    ...
                    <nav className="text-sm text-gray-500">
                        ...
                        <Link href="/xsmt" className="hover:text-gray-700">XSMT</Link>
                        <span className="mx-2 text-gray-300">›</span>
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">Kết quả XSMT ngày {date}</h1>
                            <p className="text-gray-500 text-sm mt-1 capitalize">{dateVN}</p>
                        </div>
                        <div className="flex gap-2 shrink-0 mt-1">
                            {prevDate && (
                                <Link href={`/xsmt/${prevDate}`} className="text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:border-amber-300 hover:text-amber-700 transition-colors">
                                    ← {prevDate}
                                </Link>
                            )}
                            {nextDate && (
                                <Link href={`/xsmt/${nextDate}`} className="text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:border-amber-300 hover:text-amber-700 transition-colors">
                                    {nextDate} →
                                </Link>
                            )}
                        </div>
                    ...
                    {draws.length > 0 ? (
                        ...
                        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-10 text-center">
                            ...
                            <Link href="/xsmt" className="inline-block mt-4 text-sm text-amber-600 hover:text-amber-700 font-medium">
