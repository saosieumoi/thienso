...
export async function generateMetadata(): Promise<Metadata> {
    ...
    return {
        title: `XSMT hôm nay ${dateStr} — Kết quả xổ số Miền Trung — Thiên Số`,
        description: `Kết quả XSMT hôm nay ${dateStr}. Đầy đủ các đài: Đà Nẵng, Khánh Hòa, Bình Định... Bảng lô tô, dò vé nhanh. Cập nhật ngay sau 17:15.`,
        openGraph: {
            title: `KQXSMT hôm nay ${dateStr} — Thiên Số`,
            description: 'Kết quả xổ số Miền Trung nhanh nhất, đầy đủ nhất',
        },
    }
}

// ── Fetch data ────────────────────────────────────────
async function getXSMTData() {
...
// ── Page ─────────────────────────────────────────────
export default async function XSMTPage() {
    const { draws, history, latestDate } = await getXSMTData()
            ...
            <div className="flex gap-6">
                ...
                <main className="flex-1 space-y-5 min-w-0">
                    ...
                    <nav className="text-sm text-gray-500">
                        ...
                        <span className="text-gray-800 font-medium">XSMT</span>
                    </nav>
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">
                                Kết quả XSMT {isToday ? 'hôm nay' : 'gần nhất'}
                            </h1>
                    ...
                    {draws.length > 0 ? (
                        ...
                        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-10 text-center">
                            ...
                            <p className="text-sm mt-2 text-blue-500">
                                XSMT quay lúc 17:15 — trang tự cập nhật sau khi có kết quả
                            </p>
                        </div>
                    )}
                    ...
                    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                        <div className="bg-gray-50 px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                            ...
                            <a href="/xsmt/lich-su" className="text-xs text-amber-600 hover:text-amber-700 font-medium">
                                Xem thêm →
                            </a>
                        </div>
                        <div className="overflow-x-auto">
                                    ...
                                    {history.map(h => {
                                        ...
                                        return (
                                            <tr key={h.id} className="border-t border-gray-50 hover:bg-amber-50/40 transition-colors">
                                                ...
                                                <td className="px-5 py-3 text-gray-600 font-medium">{h.province?.name ?? '—'}</td>
                                                <td className="px-5 py-3 font-bold text-amber-700 text-base tracking-wide">{dbNum}</td>
                        ...
                        <script
                            ...
                            dangerouslySetInnerHTML={{
                                __html: JSON.stringify({
                                    ...
                                    organizer: { '@type': 'Organization', name: 'Công ty XSKT Miền Trung' },
                                })
                            }}
                        />
                    )}
