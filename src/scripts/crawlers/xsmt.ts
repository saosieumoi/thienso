...
//
// Khác XSMN:
//   - class table: table-xsmn livetn2 (XSMN là livetn3)
//   - th a[title] = tên đài trực tiếp: "Đà Nẵng" (không có prefix "Xổ số ")
//
// Mapping giải (giống XSMN — thứ tự row trong tbody):
...
// XSMT: title trong th a là tên trực tiếp, không có "Xổ số "
...
function tailNum(s: string): number { return parseInt(norm(s).slice(-2), 10) }
...
function validate(prizes: ParsedPrize[], code: string): { valid: boolean; reason?: string } {
    if (prizes.length < 7) return { valid: false, reason: `${code}: chỉ có ${prizes.length}/7 giải` }
...
function parseDaiphat(html: string): ParsedProvince[] {
    ...
    if (!table.length) {
        console.warn('[XSMT] table.table-xsmn not found')
        return []
    }
    ...
    table.find('thead th').each((i, th) => {
        ...
        const fromTitle = (a.attr('title') ?? '').toLowerCase().trim()
        const fromTitleStripped = fromTitle.replace(/^xổ số /i, '').trim()
        const fromText = a.text().toLowerCase().trim()
        const code = NAME_TO_CODE[fromTitle]
            ?? NAME_TO_CODE[fromTitleStripped]
            ?? NAME_TO_CODE[fromText]
        if (code) {
            codes.push(code)
        } else {
            console.warn(`[XSMT] Không map được đài: "${a.attr('title')}"`)
        }
    })

    if (codes.length === 0) {
        console.warn('[XSMT] Không tìm thấy đài nào trong thead')
        return []
    }
    ...
    table.find('tbody tr').each((rowIdx, row) => {
        ...
        $(row).find('td.tn_prize').each((colIdx, td) => {
            ...
            if (numbers.length > 0) prizeMap[code].push({ name: prizeName, numbers })
        })
    })
...
async function insertProvince(
    ...
    if (!lotteryType) throw new Error('LotteryType XSMT not found — chạy seed chưa?')

    const prov = await prisma.province.findUnique({ where: { code: province.code } })
    if (!prov) { console.warn(`  Province ${province.code} not in DB, skip`); return 0 }
...
export async function crawlXSMT(date?: Date): Promise<CrawlResult> {
    ...
    for (const province of provinces) {
        ...
        if (!valid) { console.warn(`  ⚠️  ${reason} — skip`); continue }
    ...
    console.log(`[XSMT] ✅ Done: ${total} records, ${provinces.length} đài`)
    return { success: true, drawDate: dateStr, recordsInserted: total, source: 'xosodaiphat.com' }
}